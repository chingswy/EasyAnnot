...
def index():
    ...
    first_images = [(sub, os.path.join(sub, app.config['imgnames'][sub][frame])) for sub in subs]
    return render_template('index_match_group.html', first_images=first_images, frame=frame)
...
def query_points():
    ...
    if os.path.exists(points_name):
        ...
        points = {sub: {0: []} for sub in app.config['subs']}
...
def triangulate():
    ...
    print(datas)
    import numpy as np
    from easymocap.mytools.camera_utils import read_cameras, Undistort
    ...
    ret = {key: {} for key in datas.keys()}
    for group in datas[cams[0]]:
        records2d = np.zeros((len(cams), 1000, 3))
        colors = {
            key: '#000000' for key in range(100)
        }
        max_id = -1
        for nv, sub in enumerate(cams):
            data = datas[sub][group]
            for annot in data:
                pid = annot['id']
                if pid > max_id:
                    max_id = pid
